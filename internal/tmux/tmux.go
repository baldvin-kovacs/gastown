...
func (t *Tmux) KillSessionWithProcesses(name string) error {
	...
	if pid != "" {
		...
		if pgid != "" && pgid != "0" && pgid != "1" {
			// Kill process group using killProcessGroup() directly, rather than shelling
			...
			// killProcessGroup with positive PGID targets the process group (POSIX).
			pgidInt, _ := strconv.Atoi(pgid)
			_ = killProcessGroup(pgidInt, sigTERM)
			time.Sleep(100 * time.Millisecond)
			_ = killProcessGroup(pgidInt, sigKILL)
		}
...
func (t *Tmux) KillPaneProcesses(pane string) error {
	...
	pgid := getProcessGroupID(pid)
	if pgid != "" && pgid != "0" && pgid != "1" {
		// Kill process group using killProcessGroup() directly.
